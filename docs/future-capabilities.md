...
## Vendor-numbered sheet grammar carve-out (per ADR-0027, issue #72)

- Some sheets carry vendor/consultant drawing numbers instead of the firm's own sheet-number grammar — lighting-consultant photometric sheets (`267097-8 A1`), or bookmarks that are literally a page-ordinal (`1OF3`). Both channels fail `_SHEET_CORE` by design, so today they land in `parse_anomaly` quarantine at `status='evidence'` (ADR-0027 / #65) — correct, since these tokens genuinely aren't the firm's sheet-number grammar and the Reviewer needs to adjudicate them by eye regardless.
- Decided **against** building a `vendor_numbered_sheet` carve-out (a fallback that reads the page's title block for a stable token and records it as an unreconcilable-but-countable catalog entry) for now: it would only improve completeness-sweep bookkeeping (letting the sweep count these sheets per volume instead of leaving them fully outside reconciliation), not correctness — the Reviewer still has to eyeball every one. Given every fixture explored this session (#77–#85) turned up multiple new detection-precision failure modes on real projects, adding a NEW extraction path here — with its own false-positive risk on title-block noise — isn't worth it until parse_anomaly quarantine itself proves to be a real workflow friction point (e.g. Reviewers routinely re-dismissing the same class of vendor-numbered sheet across projects).
- If revisited: scope the carve-out narrowly (title-block token extraction, only for bookmarks that already failed `_SHEET_CORE`), and calibrate against Artisan Prescot Village's labeled example (`Civil and Landscape.pdf` pp.67-69, Drawing No. `267097-8 A1-A3`) plus at least one other project before shipping.
